  ...
  return (
    <footer className="relative w-full overflow-hidden border-t border-white/10 bg-gradient-to-b from-[#0b1f45] via-[#071633] to-[#020b1b] font-lexend">
      ...
      <Container className="relative z-10 pb-14 pt-10 md:pb-16 md:pt-14">
        ...
        <div className="grid gap-3 sm:gap-5 pb-8 grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
          {data.columns.map((col) => (
            <div
              key={col.title}
              className="rounded-2xl p-4 sm:p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-sm"
            >
              <h3 className="text-[18px] sm:text-[26px] font-semibold text-white">{col.title}</h3>
              ...
              <ul className="space-y-2.5">
                {col.links.map((l) => {
                  ...
                  const className = isMailto
                    ? "text-[13px] sm:text-[15px] font-[500] text-[var(--brand-light)] underline decoration-dotted underline-offset-4 transition-colors hover:text-white"
                    : "text-[13px] sm:text-[15px] font-[500] text-slate-200/90 transition-colors hover:text-[var(--brand-light)]";
